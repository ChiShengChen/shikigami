...
根據式神類型，選用對應的 prompt 結構。讀取 `references/templates.md` 取得完整模板。
...
### 第四步：附加護法（Optional Enhancements）

根據需要，為 prompt 加入：

- **Self-check 指令**：讓式神在完成後自行檢查產出
- **Progress reporting**：讓式神回報進度
- **Graceful degradation**：當部分工具不可用時的降級策略
- **Chain-of-thought 觸發**：在關鍵決策點加入「先想清楚再行動」的提示
...
當需要多式神協作時，參考 `references/formations.md` 了解常見陣法模式。
